...
export default async function GalleryPage({ searchParams }: Props) {
  ...
  let query = supabase
    ...
    .order("product_code")
    .order("resolution_type") // 'high' < 'low' alphabetically — low preferred for thumb
    .order("position");
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        <div className="flex-1">
          <h2 className="text-lg font-semibold text-gray-900">Galeria de produtos</h2>
        ...
        <form method="GET" action="/gallery" className="flex gap-2">
          <input
            ...
            className="w-52 rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand focus:ring-1 focus:ring-brand transition"
          />
          <button
            type="submit"
          ...
          {q && (
            <Link
              href="/gallery"
              className="border border-gray-300 hover:border-gray-400 text-gray-600 text-sm px-3 py-2 rounded-lg transition"
            >
              Limpar
            </Link>
          )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {products.map(([code, { thumbUrl, count, hasHigh, hasLow }]) => (
            <Link
              ...
              className="group bg-white border border-gray-200 rounded-xl overflow-hidden hover:border-brand hover:shadow-md transition"
            >
              {/* Thumbnail */}
